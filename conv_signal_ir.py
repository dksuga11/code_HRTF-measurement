...
if __name__ == "__main__":
    ...
    if ext == "wav":
        ir_ear_l, _ = sf.read(f"{folderpath}/IR_ear-l.wav")
        ir_ear_r, _ = sf.read(f"{folderpath}/IR_ear-r.wav")
    # インパルス応答をバイナリファイルで入力する場合------
    if ext == "npy":
        ir_ear_l = np.load(f"{folderpath}/IR_ear-l.npy")
        ir_ear_r = np.load(f"{folderpath}/IR_ear-r.npy")

    in_sig, fs = sf.read(filepath_sound)

    out_sig_l = conv_signal_and_ir(in_sig, ir_ear_l)
    out_sig_r = conv_signal_and_ir(in_sig, ir_ear_r)
    ...
    sf.write(
        file=f"{folderpath}/{filename}_convolve_ir.wav", data=out_sig, samplerate=fs
    )
